...
void KernelLauncher(const void* inp_data, const int N, void* out_data)
{
    ...
    PimCopyMemory(pim_input->data, (void*)inp_data, sizeof(half) * N, DEVICE_TO_PIM);

    DLOG(INFO) << "Calling PIMExecuteRelu";
    /* __PIM_API__ call : Execute PIM kernel (Relu) */
    PimExecuteRelu(device_output, pim_input);

    PimCopyMemory((void*)out_data, (void*)device_output->data, sizeof(half) * N, PIM_TO_DEVICE);
...
class PimReluOp : public OpKernel
{
   public:
    explicit PimReluOp(OpKernelConstruction* context) : OpKernel(context) {}
...
REGISTER_KERNEL_BUILDER(Name("PimRelu").Device(DEVICE_GPU), PimReluOp);
